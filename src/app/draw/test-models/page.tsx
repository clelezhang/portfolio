...
export default function ModelTestPage() {
  ...
  const [panels, setPanels] = useState<PanelState[]>(
    MODELS.map(() => ({ asciiBlocks: [], shapes: [], isLoading: false }))
  );
  ...
  const [drawMode, setDrawMode] = useState<'shapes' | 'ascii'>('shapes');
  ...
  const redrawPanel = useCallback((index: number) => {
    ...
    panel.shapes.forEach((shape) => {
      ctx.strokeStyle = shape.color || MODELS[index].color;
    ...
    panel.asciiBlocks.forEach((block) => {
      ...
      ctx.fillStyle = block.color || MODELS[index].color;
  ...
  const handleClear = () => {
    ...
    setPanels(MODELS.map(() => ({ asciiBlocks: [], shapes: [], isLoading: false })));
    setHumanStrokes([]);
    setCurrentStroke(null);
  };
    ...
    <div className="h-dvh w-screen flex flex-col bg-white">
        ...
        <div className="absolute bottom-20 right-4 bg-white border border-gray-200 rounded-lg p-4 shadow-lg text-sm z-10 w-72">
          ...
          <div className="mb-3">
            ...
            <div className="flex gap-2">
              {(['shapes', 'ascii'] as const).map((mode) => (
                <button
                  key={mode}
                  onClick={() => setDrawMode(mode)}
                  className={`px-3 py-1 text-xs rounded-full ${drawMode === mode ? 'bg-black text-white' : 'bg-gray-100 hover:bg-gray-200'}`}
                >
                  {mode}
                </button>
              ))}
      ...
      <div className="flex-1 flex">
        {MODELS.map((model, index) => (
          <div key={model.id} className="flex-1 flex flex-col border-r border-gray-200 last:border-r-0">
            {/* Model label */}
            <div
              className="p-2 border-b border-gray-100 text-center text-xs font-medium uppercase tracking-wide flex items-center justify-center gap-2"
      ...
      <div className="flex flex-col items-center gap-2 p-4 border-t border-gray-100">
        <div className="flex gap-2 items-center">
          ...
          <div className="flex items-center gap-1 px-2 py-1 border border-gray-200 rounded-full">
            <button
              onClick={() => setDrawMode('shapes')}
              className={`px-2 py-1 text-xs rounded-full ${drawMode === 'shapes' ? 'bg-black text-white' : 'hover:bg-gray-100'}`}
            >
              shapes
            </button>
            <button
              onClick={() => setDrawMode('ascii')}
              className={`px-2 py-1 text-xs rounded-full ${drawMode === 'ascii' ? 'bg-black text-white' : 'hover:bg-gray-100'}`}
            >
              ascii
            </button>
          </div>
