...
const MODELS: CompareModel[] = [
  ...
  { id: 'sonnet', label: 'Sonnet 4', color: '#3b82f6' },
  { id: 'sonnet-4.6', label: 'Sonnet 4.6', color: '#22c55e' },
];

export default function Test46Page() {
  ...
  return (
    <div className="draw-page">
      ...
      {/* Action buttons - positioned above the toolbar */}
      <div className="absolute bottom-16 right-4 z-50 flex items-center gap-2">
        <button
          ...
          className="px-4 py-2 bg-black text-white rounded-full text-sm hover:bg-gray-800 disabled:opacity-50 disabled:cursor-not-allowed shadow-lg"
        >
          {panels.some((p) => p.isLoading) ? '...' : 'Go'}
        </button>
        <button
          onClick={handleClear}
          className="px-3 py-2 bg-white border border-gray-200 rounded-full text-sm hover:bg-gray-50 shadow-lg"
        >
          Clear
        </button>
        <button
          onClick={() => setShowSettings(!showSettings)}
          className="px-3 py-2 bg-white border border-gray-200 rounded-full text-sm text-gray-400 hover:text-gray-600 shadow-lg"
        >
          Settings
        </button>
      </div>
