...
type ApproachType = 'image' | 'svg';

const APPROACHES: { id: ApproachType; label: string; color: string; description: string }[] = [
  { id: 'image', label: 'IMAGE-BASED', color: '#ef4444', description: 'Claude receives canvas as image every turn' },
  { id: 'svg', label: 'HYBRID SVG', color: '#22c55e', description: 'Image+SVG on sync turns, SVG only between syncs' },
];

export default function SvgVsImageTestPage() {
  ...
  const [prompt, setPrompt] = useState(
    `You are drawing on a shared canvas with a human. Add to what's there. Create something beautiful and detailed.`
  );
  ...
  const [hybridUsedImage, setHybridUsedImage] = useState(false); // Track if hybrid used image this turn
  const [lastSyncContext, setLastSyncContext] = useState<{ observation: string; intention: string; turn: number } | null>(null);
  ...
  // Calculate stroke stats whenever strokes change
  useEffect(() => {
  ...
  // Prepare canvas for image capture - includes background, Claude shapes, and human strokes
  const prepareCanvasForCapture = (canvas: HTMLCanvasElement, panelIndex: number) => {
    ...
    // 1. Fill with white background (JPEG transparency = black otherwise)
    ctx.fillStyle = '#ffffff';
    ctx.fillRect(0, 0, canvas.width, canvas.height);

    // 2. Draw Claude's existing shapes
    panel.shapes.forEach((shape) => {
      ctx.strokeStyle = shape.color || '#3b82f6';
      ctx.lineWidth = shape.strokeWidth || 2;
      ctx.lineCap = 'round';
    ...
    // 5. Draw current stroke if in progress
    if (currentStroke) {
  ...
  const handleYourTurn = async () => {
    ...
    // Determine if hybrid should use image this turn
    // Use image on: turn 1, or every syncInterval turns after last sync
    const shouldHybridUseImage = newTurnCount === 1 || (newTurnCount - lastSyncTurn) >= syncInterval;
    if (shouldHybridUseImage) {
      setLastSyncTurn(newTurnCount);
    }
    setHybridUsedImage(shouldHybridUseImage);

    // Set all panels to loading
    setPanels((prev) => prev.map((p) => ({ ...p, isLoading: true, responseTime: undefined })));

    // Prepare canvases for image capture
    canvasRefs.current.forEach((canvas, i) => {
      if (canvas) {
        prepareCanvasForCapture(canvas, i);
      }
    });

    // Get simplified strokes for SVG-based approach
    const simplifiedStrokes = humanStrokes.map((stroke) => ({
      ...stroke,
      d: simplifyPath(stroke.d, simplifyEpsilon),
    }));

    // Send requests in parallel for each approach
    const requests = APPROACHES.map(async (approach, index) => {
      ...
      try {
        if (approach.id === 'image') {
          // IMAGE-BASED: Always send canvas as image
          ...
          // HYBRID SVG: Always use draw-svg API, include image on sync turns
          const image = shouldHybridUseImage ? canvas.toDataURL('image/jpeg', 0.7) : undefined;
          ...
          const response = await fetch('/api/draw-svg', {
            ...
            body: JSON.stringify({
              ...
              streaming: true,
              model: 'sonnet',
              image, // Include image on sync turns for visual verification
              // Pass last sync context for SVG-only turns
              lastSyncContext: shouldHybridUseImage ? undefined : lastSyncContext,
            }),
          });

          await processStream(response, index, startTime, {
            isSyncTurn: shouldHybridUseImage,
  ...
  const processStream = async (
    ...
    index: number,
    startTime: number,
    options?: { isSyncTurn?: boolean; turnNumber?: number }
  ) => {
    ...
    let capturedIntention: string | undefined;
    ...
    // If this is the hybrid panel (index 1) and it was a sync turn, save the context
    // so Claude can reference it in subsequent SVG-only turns
    if (index === 1 && options?.isSyncTurn && (capturedObservation || capturedIntention)) {
      setLastSyncContext({
        observation: capturedObservation || '',
        intention: capturedIntention || '',
        turn: options.turnNumber || 0,
      });
    }
  };
  ...
  const tokenSavings =
    panels[0].inputTokens && panels[1].inputTokens
      ? Math.round((1 - panels[1].inputTokens / panels[0].inputTokens) * 100)
      : null;
    ...
    <div className="h-dvh w-screen flex flex-col bg-white">
      ...
      <div className="p-2 border-b border-gray-100 flex items-center justify-between text-xs">
        <div className="flex items-center gap-3">
          <span className="font-medium">IMAGE vs HYBRID SVG</span>
          <span className="text-gray-400">
            Turn {turnCount} | Sync every {syncInterval} turns
          </span>
          {turnCount > 0 && (
            <span className={`px-2 py-0.5 rounded text-[10px] ${hybridUsedImage ? 'bg-amber-100 text-amber-700' : 'bg-green-100 text-green-700'}`}>
              Hybrid: {hybridUsedImage ? 'IMAGE+SVG' : 'SVG only'}
            </span>
          )}
          {lastSyncContext && !hybridUsedImage && (
            <span className="px-2 py-0.5 rounded text-[10px] bg-blue-100 text-blue-700">
              Using context from turn {lastSyncContext.turn}
            </span>
          )}
        </div>
        <div className="flex items-center gap-4">
          {strokeStats && (
            <div className="text-gray-500">
              Strokes: {humanStrokes.length} | Simplified: {strokeStats.reduction}%
            </div>
          )}
          {tokenSavings !== null && (
            <div className={`font-medium ${tokenSavings > 0 ? 'text-green-600' : 'text-red-600'}`}>
        ...
        <div className="absolute bottom-20 right-4 bg-white border border-gray-200 rounded-lg p-4 shadow-lg text-sm z-10 w-72">
          ...
          <div className="mb-4">
            <label className="flex justify-between text-gray-600 mb-1">
              <span>Hybrid: image sync interval</span>
            ...
            <div className="flex justify-between text-xs text-gray-400">
              <span>frequent (more accurate)</span>
              <span>rare (more savings)</span>
            </div>
            <p className="text-[10px] text-gray-400 mt-1">
              Turn 1 always uses image. Next sync: turn {lastSyncTurn + syncInterval}
            </p>
          </div>
          ...
          <div className="mb-4">
            <label className="flex justify-between text-gray-600 mb-1">
              <span>Path simplification (epsilon)</span>
          ...
          {/* Editable Prompt */}
          <div>
            <label className="text-gray-600 mb-1 block">Prompt (shared by both)</label>
            <textarea
              value={prompt}
              onChange={(e) => setPrompt(e.target.value)}
              className="w-full h-24 px-2 py-1 border border-gray-200 rounded text-xs resize-none focus:outline-none focus:border-gray-400"
            />
          </div>
        </div>
      )}

      {/* Two panels - IMAGE vs SVG */}
          ...
          <div key={approach.id} className="flex-1 flex flex-col border-r border-gray-200 last:border-r-0">
            ...
            >
              ...
              <div className="text-gray-400 font-normal normal-case tracking-normal text-[10px]">
                {approach.id === 'svg' && turnCount > 0
                  ? hybridUsedImage
                    ? `Sync turn - sent IMAGE + SVG (next SVG-only: turns ${turnCount + 1}-${lastSyncTurn + syncInterval - 1})`
                    : `SVG only (next sync: turn ${lastSyncTurn + syncInterval})`
      ...
      <div className="flex flex-col items-center gap-2 p-4 border-t border-gray-100">
        ...
        <p className="text-xs text-gray-400">
          Draw on the left panel. Both approaches see the same input and use the same prompt.
        </p>
      </div>
    </div>
  );
