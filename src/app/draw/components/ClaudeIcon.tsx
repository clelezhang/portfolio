...
const PHASES = [
  ...
  { id: 'cardinals-grow', duration: 0.3 },
  { id: 'diagonals-catch-up', duration: 0.2 },
  { id: 'rays-grow', duration: 0.15 },
...
const FACE_APPEAR_THRESHOLD = 0.25;
...
export function ClaudeIcon({ size = 24, isLoading = false, onClick, className = '', disabled = false }: ClaudeIconProps) {
  ...
  const faceVisibility = isReverse
    ? { show: directMorphT >= FACE_APPEAR_THRESHOLD, opacity: 1 }
    : { show: p <= 3 || p === 0, opacity: 1 };
  ...
  } else if (isReverse && directMorphT >= FACE_APPEAR_THRESHOLD) {
    // Momentum wobble after face appears
    const t = (directMorphT - FACE_APPEAR_THRESHOLD) / (1 - FACE_APPEAR_THRESHOLD);
